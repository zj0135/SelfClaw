...
{
    [Fact]
    public async Task Continuation_requires_a_checkpoint_before_preparing_any_tools()
    {
        ...
        request = request with { ExecutionContext = new(DirectTurnOrigin.Continuation,
            new DirectCapabilityCeiling("system", [], [], [], []), null) };

        var events = await CollectAsync(CreateRuntime(factory).StreamTurnAsync(request));

        factory.CreateCalls.Should().Be(0);
        events.OfType<RunCompletedEvent>().Should().ContainSingle().Which.ErrorMessage.Should().Contain("checkpoint");
    }
    ...
    public async Task Default_model_is_prepared_before_subagent_tools_capture_the_parent_model()
    {
        ...
        request = request with { Agent = request.Agent with { SubagentIds = ["reviewer"] } };

        var events = await CollectAsync(CreateRuntime(factory, resolver).StreamTurnAsync(request));

        factory.ScopeCalls.Should().ContainSingle();
        factory.LastTools.Select(tool => tool.Name).Should().Contain("delegate_to_subagent");
        events.OfType<RunCompletedEvent>().Should().ContainSingle().Which.Status.Should().Be(RunCompletionStatus.Succeeded);
    }

    [Fact]
    public async Task Invalid_model_fails_before_capability_resolution_or_client_creation()
    {
        var factory = new FakeChatClientFactory(new ScriptedChatClient([]))
        {
            PreparationException = new InvalidOperationException("disabled model")
        };
        var resolver = new FakeCapabilityResolver(new DirectTurnCapabilityLease([], [], new Dictionary<Guid, string>(), []));
        var events = await CollectAsync(CreateRuntime(factory, resolver).StreamTurnAsync(CreateRequest(factory.Profile.Id)));

        resolver.Calls.Should().Be(0);
        factory.CreateCalls.Should().Be(0);
        events.OfType<RunCompletedEvent>().Should().ContainSingle().Which.ErrorMessage.Should().Be("disabled model");
    }
    ...
    public async Task StreamTurnAsync_translates_all_content_and_aggregates_usage()
    {
        ...
        var request = CreateRequest(modelProfileId: factory.Profile.Id, workspace: CreateWorkspace());
    ...
    public async Task StreamTurnAsync_uses_scope_default_and_binds_workspace_tools()
    {
        ...
        factory.LastTools.Select(tool => tool.Name).Should().Equal(
    ...
    /// </summary>
    internal static DirectTurnCapabilityResolver CreateCapabilityResolver(IWorkspaceToolService? workspaceTools = null,
        ISubagentTaskCoordinator? coordinator = null)
    {
        ...
        return new DirectTurnCapabilityResolver(
            new WorkspaceAgentToolset(workspaceTools ?? new NoOpWorkspaceTools()),
            ...
            new SelfClaw.Infrastructure.Agents.Direct.Capabilities.SubagentCapabilitySource(coordinator));
    }

    internal static ChatTurnRequest CreateRequest(Guid? modelProfileId, WorkspaceRoot? workspace = null)
    {
        var conversationId = Guid.NewGuid();
        var now = DateTimeOffset.UtcNow;
    ...
    internal sealed class FakeChatClientFactory : IAiChatClientFactory
    {
        ...
        public Task<AiProviderClientRequest> PrepareAsync(
            Guid? modelProfileId,
            CancellationToken cancellationToken = default)
        {
            if (PreparationException is not null) return Task.FromException<AiProviderClientRequest>(PreparationException);
            if (modelProfileId is null) ScopeCalls.Add(AiModelSelectionScopes.DesktopDefault);
            var now = DateTimeOffset.UtcNow;
