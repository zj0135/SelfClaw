...
public sealed class DirectPromptComposerTests
{
    ...
    public void Fixed_budget_does_not_materialize_discarded_old_tool_history()
    {
        ...
        long Measure((List<MessageRecord> Messages, List<ToolExecutionRecord> Runs) history)
        {
            var before = GC.GetAllocatedBytesForCurrentThread();
            var result = composer.BuildMessages(history.Messages, history.Runs, "", [], NoAdjustments(), Interactive(), budget);
            var allocated = GC.GetAllocatedBytesForCurrentThread() - before;
            result.Should().HaveCount(3);
            return allocated;
        }

        static (List<MessageRecord>, List<ToolExecutionRecord>) CreateLongHistory(int count)
        {
            var (messages, runs) = CreateHistory(Enumerable.Range(0, count)
                .Select(index => ($"assistant-{index}", MessageRole.Assistant, MessageStatus.Completed, ""))
                .Append(("user", MessageRole.User, MessageStatus.Completed, "continue")).ToArray());
