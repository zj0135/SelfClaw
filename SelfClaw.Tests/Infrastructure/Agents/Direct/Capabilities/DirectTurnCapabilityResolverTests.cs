...
public sealed class DirectTurnCapabilityResolverTests : IDisposable
{
    ...
    public async Task Queued_capability_changes_use_the_same_rule_in_preflight_and_runtime(string change)
    {
        ...
        var ceiling = new DirectCapabilityCeiling("system", [], [new(package.Id, package.Version, package.ContentHash)], [], []);
        var modelId = request.ModelProfileId ?? throw new InvalidOperationException();
        var models = new StubAiModelCatalog(modelId) { EnabledModels = [new(modelId, "Model", "model", "Provider")] };
        ...
        {
            "disabled" => package with { IsEnabled = false },
            "upgraded" => package with { Version = "2.0.0", ContentHash = "new-hash" },
            _ => package
        };
    ...
    public async Task Changed_mcp_revision_or_enablement_is_rejected_before_child_execution(bool disable)
    {
        ...
        var request = CreateRequest([], "task", mcpServerIds: [server.Id]);
        var modelId = request.ModelProfileId ?? throw new InvalidOperationException();
        var ceiling = new DirectCapabilityCeiling("system", [], [], [new(server.Id, server.ConfigRevision)], []);
        var definition = new SubagentDefinitionSnapshot(1, "child", "Child", "", modelId, "system", [], [], [server.Id], 30, "");
        var models = new StubAiModelCatalog(modelId) { EnabledModels = [new(modelId, "Model", "model", "Provider")] };
        var start = new SubagentTaskStartRequest(request.ConversationId, request.TurnId, "child", "task", request.Agent,
            modelId, null, ToolPermissionMode.FullAccess, ceiling);
        ...
        await child.Should().ThrowAsync<InvalidDataException>().WithMessage("*MCP*could not be loaded*");
        await using var continuation = await resolver.ResolveAsync(request with { ExecutionContext = new(DirectTurnOrigin.Continuation, ceiling, null) });
        continuation.Tools.Should().BeEmpty();
    }
    ...
    public async Task ResolveAsync_rejects_mcp_tool_names_that_collide_after_provider_name_normalization()
    {
        ...
        await action.Should().ThrowAsync<InvalidDataException>()
            .WithMessage("*Direct tool name collision*mcp__fixture__a_b*");
        manager.ReleasedLeaseCount.Should().Be(1);
    }
