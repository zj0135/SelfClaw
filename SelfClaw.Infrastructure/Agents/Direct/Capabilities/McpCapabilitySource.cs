...
internal sealed class McpCapabilitySource
{
    ...
    public async Task<McpCapabilities> ResolveAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        MergeDiagnostics(resolutions, diagnostics, _stateChangeNotifier);
        return new McpCapabilities(resolutions
            .Where(resolution => resolution.Connected)
            .Select(resolution => new DirectMcpCapability(resolution.Server.Id, resolution.Server.ConfigRevision))
            .ToArray(), resolutions.SelectMany(resolution => resolution.Tools).ToArray());
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!configuration.IsAvailable)
        {
            ...
            var configHealthRecorded = await TryRecordHealthAsync(
                    ...
                    cancellationToken)
                .ConfigureAwait(false);
            return new McpServerResolution(server, false, messages, degradations, configHealthRecorded, []);
        }
        ...
        catch
        {
            ...
            var failureHealthRecorded = await TryRecordHealthAsync(
                    ...
                    cancellationToken)
                .ConfigureAwait(false);
            return new McpServerResolution(server, false, messages, degradations, failureHealthRecorded, []);
        }
        ...
        return new McpServerResolution(
            ...
            degradations,
            healthRecorded,
            lease.Tools.Select(tool => _toolAdapter.Create(tool, configuration)).ToArray());
    }
