...
<template>
	<div class="toolbar">
		<label class="search">
			...
			<input :value="modelValue" type="search" placeholder="搜索名称、ID 或描述" @input="$emit('update:modelValue', $event.target.value)" />
...
button { display: inline-flex; align-items: center; justify-content: center; height: 36px; border: 1px solid var(--sc-line-2); border-radius: 6px; background: var(--sc-panel); color: var(--sc-soft); }
.icon-button { width: 36px; padding: 0; }
.clear { width: 24px; height: 24px; padding: 0; border: 0; background: transparent; }
.primary { gap: 7px; padding: 0 12px; border-color: var(--sc-acid); background: var(--sc-acid); color: var(--sc-acid-ink); }
button:disabled { cursor: wait; opacity: 0.55; }
.spin { animation: sc-spin 0.8s linear infinite; }
</style>
