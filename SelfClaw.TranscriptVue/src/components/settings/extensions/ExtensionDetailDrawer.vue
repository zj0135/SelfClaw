...
<template>
	<aside class="drawer">
		<header>
			...
			<button type="button" class="icon" title="关闭详情" @click="$emit('close')">
				<X :size="17" />
			</button>
		</header>
		...
		<dl>
			...
			<template v-if="item.transport">
				<dt>传输</dt>
				<dd>{{ item.transport === 1 ? 'HTTP' : item.transport === 0 ? 'STDIO' : item.transport }}</dd>
			</template>
			<template v-if="item.sourcePluginId">
				<dt>来源插件</dt>
				<dd>{{ item.sourcePluginId }}</dd>
			</template>
			<template v-if="item.lastCheckedAtUtc">
				<dt>最近检查</dt>
				<dd>{{ new Date(item.lastCheckedAtUtc).toLocaleString() }}</dd>
			</template>
			<template v-if="item.lastError">
				<dt>最近错误</dt>
				<dd class="error">{{ item.lastError }}</dd>
			</template>
		...
		<footer v-if="kind === 'mcpServer' || !item.sourcePluginId">
			...
			<button v-if="!item.sourcePluginId" type="button" class="danger" :disabled="pending"
				@click="$emit('delete')">
				<Trash2 :size="14" />删除
			</button>
		</footer>
