...
<template>
	<section>
		<div class="section-head">
			<div>
				<KeyRound :size="14" /><strong>{{ title }}</strong>
			</div>
			<button v-if="!managed" type="button" title="添加字段" @click="$emit('add')">
				<Plus :size="14" />
			</button>
		</div>
		<div v-for="(row, index) in rows" :key="index" class="entry">
			<input v-model="row.key" class="key" placeholder="键名" :disabled="managed" />
			<input v-model="row.value" :type="row.isSecret ? 'password' : 'text'"
				:placeholder="row.isSecret && row.hasSecret ? '已配置，留空保留' : '值'" :disabled="row.clearSecret" />
			...
			<button v-if="!managed" type="button" class="remove" title="删除字段" @click="$emit('remove', index)">
				<Trash2 :size="14" />
			</button>
		</div>
...
.entry {
	display: grid;
	grid-template-columns: minmax(100px, .8fr) minmax(130px, 1fr) auto auto 28px;
	align-items: center;
	gap: 6px;
}
