...
<template>
	<div class="row" :class="{ selected, pending }">
		...
		<label class="switch" :title="item.sourcePluginId ? `由 ${item.sourcePluginId} 管理` : item.enabled ? '停用' : '启用'">
			<input type="checkbox" :checked="item.enabled" :disabled="pending || Boolean(item.sourcePluginId)"
...
.row {
	display: grid;
	grid-template-columns: minmax(0, 1fr) 38px 20px;
	align-items: center;
	min-height: 88px;
...
.row.selected {
	box-shadow: inset 2px 0 var(--sc-acid);
}
